...
describe('view orientation (P49)', () => {
  it('turns the board contents clockwise', () => {
    ...
    expect(toScreenXY(0, 0, 1)).toEqual([N, 0]);
    expect(toScreenXY(0, 0, 2)).toEqual([N, N]);
    expect(toScreenXY(0, 0, 3)).toEqual([0, N]);
    expect(toScreenXY(0, 0, 4)).toEqual([0, 0]);
  });
  ...
  it('puts every seat’s home corner bottom-right at its own view turns', () => {
    ...
    for (const c of COLOR_ORDER) {
      const [x, y] = HOME[c];
      expect(toScreenXY(x, y, TURNS_TO_BOTTOM_RIGHT[c])).toEqual([N, N]);
    }
  });
  ...
  it('round-trips screen ↔ board for every cell and every turn', () => {
    ...
    for (let t = -4; t <= 7; t++) {
      for (let i = 0; i < BOARD_SIZE * BOARD_SIZE; i++) {
        ...
        const [sx, sy] = toScreenXY(x, y, t);
        expect(toBoardXY(sx, sy, t)).toEqual([x, y]);
        expect(toScreenIndex(i, t)).toBe(sy * BOARD_SIZE + sx);
      }
    }
  });
  ...
  it('is a permutation — no cell is dropped or doubled up', () => {
    for (const t of [1, 2, 3]) {
      const seen = new Set(
        Array.from({ length: BOARD_SIZE * BOARD_SIZE }, (_, i) => toScreenIndex(i, t)),
      );
  ...
  it('re-indexes a board array without copying at rest', () => {
    ...
    board[0] = 'blue';
    expect(toScreenBoard(board, 0)).toBe(board);
    // Blue's corner sits bottom-right at its own two turns.
    expect(toScreenBoard(board, 2)[N * BOARD_SIZE + N]).toBe('blue');
    expect(toScreenBoard(board, 2)[0]).toBeNull();
  });
});
